...
# --- 🔒 密码保护配置 (请在此处修改您的密码) ---
# 建议设置一个复杂的密码，不要告诉无关人员
PASSWORD = "yyw@25zgykdx"  # <--- 在这里修改您的专属密码
...
def check_password():
    ...
    def password_entered():
        if st.session_state["password"] == PASSWORD:
            ...
            del st.session_state["password"]  # 不保留明文密码
        else:
...
with st.sidebar:
    ...
    current_warehouse = st.radio("选择当前查看的仓库:", warehouses, index=1)
...
# --- 主界面逻辑 ---
# (此处逻辑与之前完全一致，为节省篇幅省略重复部分，实际使用时请确保完整代码包含下方所有 render 函数)
# 为了保持回答简洁，我假设您会将之前提供的完整 render 函数代码粘贴在这里。
# ⚠️ 重要：请务必将之前我提供的完整代码中 `render_inventory_view` 到 `render_status` 的所有函数
# 以及底部的路由逻辑 (if choice == ...) 全部复制到这里下面。
...
def render_add_item():
    st.header("➕ 新增/入库")
    if not db.token: st.error("请先配置 GitHub Token。"); return
    with st.form("add_form"):
        c1, c2 = st.columns(2)
        with c1:
            brand = st.selectbox("品牌", get_default_brands(), index=0, key="add_brand")
            if st.checkbox("输入新品牌", key="cb_brand"): brand = st.text_input("新品牌名称", value=brand, key="txt_brand")
            item = st.selectbox("物品名称", get_default_items(), index=0, key="add_item")
            if st.checkbox("输入新物品", key="cb_item"): item = st.text_input("新物品名称", value=item, key="txt_item")
        ...
        if submitted:
            if not brand or not item:
                ...
                if df[mask].empty:
                    ...
                    new_row = pd.DataFrame([{'id': new_id, 'warehouse': warehouse, 'brand': brand, 'item': item,
                ...
                if success:
                    st.success(msg); db.add_history("新增/入库", msg); st.rerun()
                else:
                    st.error(err)
    ...
    if selected:
        ...
        if st.button("更新", key="upd_btn"):
            ...
            if not df[mask].empty:
                ...
                success, err = db.save_data(df)
                if success:
                    st.success("更新成功"); db.add_history("修改数量", f"{b}-{i} -> {new_qty}"); st.rerun()
                else:
                    st.error(err)
            else:
                st.warning("物品未找到"); st.rerun()
    ...
    if src_sel and dst_wh != src_wh:
        ...
        if st.button("执行调拨", key="trans_btn"):
            ...
            if not df[mask_src].empty and df.loc[mask_src, 'quantity'].values[0] >= qty:
                ...
                if df.loc[mask_src, 'quantity'].values[0] <= 0: df = df[~mask_src]
                mask_dst = (df['brand'] == b) & (df['item'] == i) & (df['warehouse'] == dst_wh)
                if df[mask_dst].empty:
                    new_id = int(df['id'].max()) + 1 if not df.empty and 'id' in df.columns else 1
                    new_row = pd.DataFrame([{'id': new_id, 'warehouse': dst_wh, 'brand': b, 'item': i, 'quantity': qty,
                                             'image': '', 'updated_at': datetime.now().strftime("%Y-%m-%d %H:%M:%S")}])
                    df = pd.concat([df, new_row], ignore_index=True)
                else:
                ...
                success, err = db.save_data(df)
                if success:
                    st.success(f"调拨成功：{qty} 件 {b}-{i} -> {dst_wh}"); db.add_history("调拨",
                                                                                    f"{qty}件 {b}-{i}: {src_wh}->{dst_wh}"); st.rerun()
                else:
                    st.error(err)
            else:
                st.error("库存不足或物品不存在。"); st.rerun()
...
def render_delete():
    ...
    df = db.load_data(force_refresh=True)
    if df.empty: st.warning("整个库存表为空！"); return
    df_wh = df[df['warehouse'] == current_warehouse]
    if df_wh.empty: st.warning(f"⚠️ **{current_warehouse}** 仓库目前没有数据。"); return
    options = [f"{row['brand']} - {row['item']} (数量: {row['quantity']})" for _, row in df_wh.iterrows()]
    if not options: st.warning("解析后的选项列表为空。"); return
    sel = st.selectbox("选择要删除的记录", options, key="del_sel")
    if sel:
        st.write(f"🎯 准备删除：**{sel}**")
        if st.button("🔴 确认删除并强制同步", type="primary", key="del_btn"):
            parts = sel.split(" (数量: ")
            if len(parts) != 2: st.error("❌ 无法解析选项格式。"); return
            b_i_part = parts[0].split(" - ")
            if len(b_i_part) != 2: st.error("❌ 无法解析品牌/物品名称。"); return
...
def render_import_excel():
    ...
    if uploaded:
        try:
            ...
            if not all(k in df_in.columns for k in ['品牌', '物品', '数量']): st.error("缺少列：品牌，物品，数量"); st.stop()
            ...
            for _, row in df_in.iterrows():
                ...
                if df_curr[mask].empty:
                    ...
                    new_row = pd.DataFrame([{'id': new_id, 'warehouse': target_wh, 'brand': row['brand'],
                                             'item': row['item'], 'quantity': int(row['quantity']),
                                             'image': row.get('image', ''),
                                             'updated_at': datetime.now().strftime("%Y-%m-%d %H:%M:%S")}])
            ...
            if success:
                st.success(f"导入完成：新增 {count_add}, 更新 {count_upd}"); db.add_history("导入",
                                                                                   f"{uploaded.name}: +{count_add}, ~{count_upd}"); st.rerun()
            else:
                st.error(err)
        except Exception as e:
            st.error(f"读取失败：{e}")
...
def render_status():
    ...
    if db.token:
        ...
        if st.button("强制刷新数据缓存"): load_data_cached.clear(); st.rerun()
        df = db.load_data()
